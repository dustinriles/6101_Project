...
df %>%
  ...
  boxplot(df$MentalHealth,df$PhysicalHealth,df$SleepTime,names=c("Poor MH","Poor PH","Sleep (hrs)"),main="# of Poor Mental/Physical Health Days (Last 30) and Hours of Sleep",horizontal=TRUE)
...
```{r}
table(df$HeartDisease)
model1 <- lm(HeartDisease~MentalHealth + BMI + GenHealth, data = df, family = "binomial")
summary(model1)

xkabledply(fit1, title = paste("model1 :", format(formula(fit1)) ) )

#testing the model

set.seed(3456)
trainIndex <- createDataPartition(df$MentalHealth, p = .8,
                                  list = FALSE,
                                  times = 1)
Train <- df[ trainIndex,]
Test <- df[-trainIndex,]
...
model_train <- lm(HeartDisease~MentalHealth, data = Train)
```
